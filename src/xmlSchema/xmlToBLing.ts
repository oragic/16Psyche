const xmlToBling = (deal_id:string,title:string,value:string,status: string,customer_name:string) =>{
  const xmlStr = `xml=<?xml version="1.0" encoding="UTF-8"?>
  <pedido>
  <cliente>
  ...
  <itens>
  <item>
  <codigo>${deal_id}</codigo>
  <descricao>${title} ${deal_id} 001</descricao>
  <un>Pç</un>
  <qtde>10</qtde>
